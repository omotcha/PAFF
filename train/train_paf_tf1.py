...
train_writer = tf.summary.FileWriter('logdir/training_set', graph, flush_secs=1)
val_writer = tf.summary.FileWriter('logdir/validation_set', flush_secs=1)
    ...
    for epoch in range(args.num_epochs):
        for rotation in args.rotations:
            ...
            for bi, bj in batches('training'):
                session.run(train, feed_dict={x: get_batch('training', x_t[bi:bj], rotation), t: y_t[bi:bj], keep_prob: args.kp})
            print('saving states per rotation')
