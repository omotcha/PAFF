...
platform: win
env: pafnucy_env
name: train_paf_tf2.py
pafnucy training using tf2-compatible syntax, just for syntax suggestion
"""
import numpy as np
import pandas as pd
...
from tfbioSupport.tfbio.data import Featurizer, make_grid, rotate
import tfbioSupport.tfbio.net
...
io_group.add_argument('--log_dir', '-l', default='D:\\AlexYu\\kdeep.ign.gbt\\pafnucy-master\\logdir\\',
                      help='directory to store tensorboard summaries')
io_group.add_argument('--output_prefix', '-o', default='D:\\AlexYu\\kdeep.ign.gbt\\pafnucy-master\\output',
...
print('\nexamples:')
for mtype, mol in [['ligand', tmp[tuple(idx1)]], ['protein', tmp[tuple(idx2)]]]:
    print(' ', mtype)
...
graph = tfbioSupport.tfbio.net.make_SB_network(isize=isize, in_chnls=in_chnls, osize=osize,
...
with tf.compat.v1.Graph().as_default():
    train_writer = tf.compat.v1.summary.FileWriter('logdir/training_set', graph, flush_secs=1)
    val_writer = tf.compat.v1.summary.FileWriter('logdir/validation_set', flush_secs=1)

# train_writer = tf.summary.create_file_writer(os.path.join(logdir, 'training_set'))
# val_writer = tf.summary.create_file_writer(os.path.join(logdir, 'validation_set'))

net_summaries, training_summaries = tfbioSupport.tfbio.net.make_summaries_SB(graph)
...
with graph.as_default():
    saver = tf.compat.v1.train.Saver(max_to_keep=args.to_keep)
...
with tf.compat.v1.Session(graph=graph) as session:
    ...
    summary_imp = tf.compat.v1.Summary()
    feature_imp = session.run(feature_importance)
    image = tfbioSupport.tfbio.net.feature_importance_plot(feature_imp)
...
for set_name, tab in predictions.groupby('set'):
    ...
    image = tfbioSupport.tfbio.net.custom_summary_image(grid.fig)
